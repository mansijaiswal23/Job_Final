...
const JobCard = ({ job, onJobClick }: JobCardProps) => {
  const getBackgroundColor = (id: string) => {
    switch (id) {
      case "1":
        return "#FFF2E5"; // light peach
      case "2":
        return "#E5FFF8"; // light teal
      case "3":
        return "#F2E8FF"; // light lavender
      case "4":
        return "#E5F1FF"; // light blue
      case "5":
        return "#FCE7F3"; // light pink
      case "6":
        return "#EDF9FE"; // light violet
      case "7":
        return "#FFF2E5"; // light peach
      case "8":
        return "#E5FFF8"; // light teal
  ...
  return (
    <div
      className="rounded-lg p-4 relative cursor-pointer hover:shadow-md transition-shadow"
      style={{ backgroundColor: getBackgroundColor(job.id) }}
    ...
    >
      ...
      <div className="mt-4">
        <div className="flex flex-wrap gap-2 mb-3">
          <span className="bg-white px-2 py-1 text-xs rounded">
            {job.location}
          </span>
          <span className="bg-violet-100 text-violet-800 px-2 py-1 text-xs rounded">
            {job.type}
          </span>
        </div>
